...
@app.route('/api/process-contrast', methods=['POST'])
def process_contrast():
    # Your processing logic for contrast adjustment
    image_file = request.files['image']
    ...
    try:
        ...
        adjusted_image = adjust_contrast(edges, contrast)
...
@app.route('/api/process-brightness', methods=['POST'])
def process_brightness():
    ...
    try:
        ...
        print('Error processing image:', e)
        return jsonify({'error': 'Internal server error'}), 500
@app.route('/api/process-thickness', methods=['POST'])
def process_thickness():
    ...
    try:
        ...
        # Apply thickness adjustment
        thickened_image = adjust_thickness(edges, thickness)

        # Encode the processed image to base64
        _, encoded_image = cv2.imencode('.png', thickened_image)
        sketch_base64 = base64.b64encode(encoded_image).decode('utf-8')
        sketch_url = f'data:image/png;base64,{sketch_base64}'
...
def adjust_thickness(image, thickness):
    # Adjust thickness of sketch lines
